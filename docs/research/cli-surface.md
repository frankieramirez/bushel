...
**Ticket:** [#3](https://github.com/frankieramirez/quayside/issues/3) · **Map:** [#1](https://github.com/frankieramirez/quayside/issues/1)
...
1. **One shape for reads: `exec → stdout JSON → decode`.** Every read the TUI needs (`ls -a`, `image ls`, `volume ls`, `network ls`, `system status`, `stats --no-stream`) supports `--format json`; `inspect` is JSON by default. The Go layer can be a single `runJSON(ctx, args...) ([]byte, error)` that decodes stdout on exit 0 and wraps stderr on non-zero. Decode into structs with `configuration`/`id`/`status` top-level fields; keep unknown-field tolerance on (default in `encoding/json`) since shapes drift.

2. **Poll; don't look for events.** There is no watch mechanism, full stop. A Bubble Tea `tick`-driven poller running `container ls -a --format json` (and `stats --no-stream --format json` when a stats pane is visible) is the state engine. Measured latency of `ls` round-trips is well under a second; 1–2 s ticks are comfortable. Diff successive snapshots keyed on `id` to synthesize the events the UI wants (created/started/stopped/deleted).

3. **The wrapper owns every long-lived subprocess.** `logs -f` never exits on container stop — tie each follow subprocess to a `context.Context`, and cancel it when the poller sees the container leave `running` (or keep it and mark the pane "container stopped; stream idle" — but never wait on it). Same for `system logs -f`.

4. **Stats need client-side computation.** JSON stats is single-shot with a cumulative `cpuUsageUsec`; the TUI must keep the previous sample per container and derive CPU% from deltas. Never invoke streaming table mode (alt-screen ANSI would fight Bubble Tea for the terminal).

5. **Exit codes are only visible at run time.** Nothing in `ls`/`inspect` records why a container stopped. If quayside wants "exited (7)" badges it must either (a) be the invoker (`run`/`start -a` propagate codes) or (b) accept `stopped` without a code. Design the model so exit code is `*int`, usually nil.

6. **Error handling = exit code + stderr regex, defensively.** 0 success; 64 usage/parse errors (bug in the wrapper's own arg construction); 1 everything else, classified by substring: `XPC connection error` / `Ensure container system service` → service down (offer to start it), `not found` → stale UI entry (trigger immediate re-poll), `invalidState` / `is running and can not be deleted` → offer `-f`. Treat classification as best-effort; always keep the raw stderr available in the UI, since phrasings vary per command and per version.
...
8. **Pin expectations to the CLI version.** Upstream guarantees output stability only within patch versions and has changed all JSON shapes at 1.0.0. On startup run `container --version`, parse `container CLI version X.Y.Z`, and (at minimum) warn on minor/major versions newer than tested; keep captured JSON fixtures per version for regression tests. Target 1.2.x now; expect to re-validate shapes on each minor bump. `container` requires macOS 26+, which bounds the support matrix nicely.

9. **Non-TTY hygiene is already decent.** Data on stdout, errors and progress on stderr, `--progress plain|none` for pulls/runs, exit codes honest. Run every subprocess without a PTY and set `--progress none` (or parse `plain` progress lines to drive a Bubble Tea progress bar for `image pull`).
